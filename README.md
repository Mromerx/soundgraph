...
- **Album recommendations.** It builds a pool of candidate albums (top albums of each seed's similar artists), scores them against every seed using TF-IDF cosine similarity over Last.fm tags, and returns them ordered by a 0–100 "posible gusto" (likely taste) score. Each recommendation shows every seed it shares tags with and its per-seed score.
...
| `POST /api/recommendations/` | Album recommendations. Body: `{"seeds": [{"artist": "...", "album": "..."}], "n_results": 5}` — 1–5 seeds, `n_results` 1–15. Returns `[{"artist", "album", "score", "matched_seed", "matched_seeds"}]`; `score` is the max TF-IDF cosine against any seed as a 0–100 percentage, `matched_seeds` lists every seed the album shares tags with. Candidates with no tag overlap are dropped. |
