...
- `POST /api/recommendations/`
  ...
  `seeds` debe tener entre 1 y 5 elementos y `n_results` debe estar entre 1 y 15.
...
- `GET /api/connections/{search_id}/`
  Progreso en vivo de una busqueda: `{status, current_depth, max_depth, bridge_artist, seed_artists, visited_per_seed, frontier_per_seed, came_from}`; agrega `path` cuando `status="found"` o `error_message` cuando `status="failed"`. Estados: `pending`, `running`, `found`, `exhausted`, `failed`. Con `?graph=full` devuelve el grafo explorado **completo** (visitados/frontera/came_from sin acotar) para visualizar toda la busqueda que llevo al puente. Por defecto el payload viene compacto (muestras) y el GET soporta revalidacion por ETag (`If-None-Match`): mientras la busqueda no cambia de estado responde `304` sin re-serializar el payload.
- `GET /api/connections/{search_id}/events`
  Stream SSE (`text/event-stream`) del progreso de la busqueda. Publica un evento `data: {payload}` en cada cambio de estado (artista descubierto, fin de nivel, pausa, resume, stop o estado final): un evento por artista, asi las esferas aparecen una por una; los artistas que vienen en una misma respuesta de la API se publican seguidos (se descubrieron a la vez). Cada evento lleva las muestras del grafo para que el frontend lo dibuje creciendo en vivo. Al llegar a un estado final el stream se cierra. El bus vive en memoria, asi que con `runserver` el hilo real del BFS publica y el frontend pinta el puente "al instante"; el poll con ETag/304 queda de respaldo.
...
- **Progreso por SSE**: el campo `payload_revision` de `ConnectionSearch` se avanza en cada save de estado (fin de nivel, found, pause, stop) y es el ETag del GET (los heartbeats no lo tocan). El hilo publica un evento por cada artista descubierto al bus en memoria (`/api/connections/{id}/events`) con las muestras del grafo (ventana de los mas recientes), asi el frontend dibuja la exploracion creciendo artista por artista al instante; el poll queda como respaldo economico (2s en running, 5s pausado, con 304 cuando nada cambio) en vez de re-enviar el payload de ~174KB cada segundo.
