...
- **Recomendación de álbumes.** Arma un pool de álbumes candidatos (álbumes top de los artistas similares a cada semilla, más los del propio artista), los puntúa contra cada semilla con similitud coseno TF-IDF sobre los tags de Last.fm y devuelve la lista ordenada por un score de "posible gusto" de 0 a 100. El score mezcla el mejor match contra una semilla (60%) con el solapamiento promedio con todas las semillas (40%), y los resultados se diversifican con relevancia máxima-marginal (MMR) para que no queden N álbumes del mismo sonido o del mismo artista. Cada recomendación muestra todas las semillas con las que comparte tags y su score por semilla.
...
El frontend lee `VITE_LANG` del `.env` de la raíz (Vite lo carga vía `envDir` en `frontend/vite.config.js`):

- `VITE_LANG=es` → interfaz en español
- `VITE_LANG=en` → interfaz en inglés (por defecto)

Cualquier otro valor cae por defecto a inglés. Elegí un solo idioma para toda la interfaz; no se soporta mezclar idiomas.
...
| `VITE_LANG` | `en` | Idioma de la interfaz del frontend: `es` (español) o `en` (inglés). Lo lee Vite desde el `.env` de la raíz. |
...
| `POST /api/recommendations/` | Recomendación de álbumes. Body: `{"seeds": [{"artist": "...", "album": "..."}], "n_results": 5}` — 1 a 5 semillas, `n_results` de 1 a 15. Devuelve `[{"artist", "album", "score", "matched_seed", "matched_seeds"}]`; `score` es la mezcla max-promedio del coseno TF-IDF contra las semillas (60% el mejor match, 40% el promedio contra todas) expresado como porcentaje 0-100, y `matched_seeds` lista todas las semillas con las que el álbum comparte tags. El pool de candidatos incluye los top álbumes de los artistas similares y los del propio artista; los resultados se ordenan por relevancia máxima-marginal (diversificando sonido y artista). Los candidatos sin solapamiento de tags se descartan. |
